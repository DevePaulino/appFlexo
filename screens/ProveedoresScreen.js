...
import {
  ...
  TextInput, Modal, Alert, Platform, Pressable, ActivityIndicator,
} from 'react-native';
...
const s = StyleSheet.create({
  ...
  colNombre: { flex: 0.22 },
  colContacto: { flex: 0.2 },
  colEmail: { flex: 0.25 },
  colTelefono: { flex: 0.15 },
  colAcciones: { flex: 0.18, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6 },
  actionBtn: {
  ...
  modalCard: {
    ...
    borderColor: '#E2E8F0', padding: 20, width: '100%', maxWidth: 480,
  },
  modalTitle: { fontSize: 18, fontWeight: '800', color: '#0F172A', marginBottom: 14 },
  fieldLabel: { fontSize: 13, color: '#475569', fontWeight: '700', marginBottom: 5 },
  fieldInput: {
  ...
  btn: {
    ...
    paddingHorizontal: 14, paddingVertical: 9, borderRadius: 8, alignItems: 'center',
  },
  ...
  const guardar = async () => {
    ...
    try {
      const url = editandoId
        ? `${API_BASE}/api/proveedores/${editandoId}`
        : `${API_BASE}/api/proveedores`;
      const method = editandoId ? 'PUT' : 'POST';
      const body = editandoId
        ? { ...form }
        : { ...form, tipo: tipoActivo };
      ...
      {loading ? (
        ...
        <ScrollView style={s.tableContainer}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ flexGrow: 1 }}>
            <View style={Platform.select({ web: { width: '100%' }, default: { minWidth: 600 } })}>
